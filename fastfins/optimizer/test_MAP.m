function [ymap, xmap] = test_MAP(mesh, mesh_dual, obs, prior, param, forward)
...
%mesh_dual.N_side+1 = sqrt(mesh_dual.N_node);
global con_f con_h 
...
toc
fprintf('Forward %4i\t Hessmult %4i\n', [con_f, con_h])


con_f = 0;
con_h = 0;
tic
ymap = get_map_matlab(mesh, obs, prior, param, forward, y_init);
toc
fprintf('Forward %4i\t Hessmult %4i\n', [con_f, con_h])
...
if param.double_param_flag
    kmap = xmap(param.K_ind);
    smap = xmap(param.S_ind);
    figure('position',[100 100 700 600]);
    subplot(2,2,1)
    switch param.type
        case{'Affine'}
            meshc(mesh_dual.gx,mesh_dual.gy,reshape(param.K_basis*kmap,mesh_dual.M_side+1,mesh_dual.N_side+1))
        otherwise
            meshc(mesh_dual.gx,mesh_dual.gy,reshape(kmap,mesh_dual.M_side+1,mesh_dual.N_side+1))
    end
    ...
    switch param.type
        case{'Affine'}
            imagesc(mesh_dual.gx,mesh_dual.gy,reshape(param.K_basis*kmap,mesh_dual.M_side+1,mesh_dual.N_side+1));
        otherwise
            imagesc(mesh_dual.gx,mesh_dual.gy,reshape(kmap,mesh_dual.M_side+1,mesh_dual.N_side+1));
    end
    set(gca,'ydir','normal');
    xlabel('x')
    ylabel('y')
    ...
    switch param.type
        case{'Affine'}
            meshc(mesh_dual.gx,mesh_dual.gy,reshape(param.S_basis*smap,mesh_dual.N_side+1,mesh_dual.N_side+1))
        otherwise
            meshc(mesh_dual.gx,mesh_dual.gy,reshape(smap,mesh_dual.N_side+1,mesh_dual.N_side+1))
    end
    if exist('prior.beta','var')
        title(['MAP - \beta = ' num2str(prior.beta)])
    else
    ...
    switch param.type
        case{'Affine'}
            imagesc(mesh_dual.gx,mesh_dual.gy,reshape(param.S_basis*smap,mesh_dual.M_side+1,mesh_dual.N_side+1));
        otherwise
            imagesc(mesh_dual.gx,mesh_dual.gy,reshape(smap,mesh_dual.M_side+1,mesh_dual.N_side+1));
    end
    set(gca,'ydir','normal');
    xlabel('x')
    ylabel('y')
