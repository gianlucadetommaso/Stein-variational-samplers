function [x, i, pdef] = newton_cg(opt_def, grad, kern, g_kern, model, data, HI)
...
while 1
    ...
    Hcd = matvec_Hx(kern, g_kern, model, data, HI, cd);
    % curvature
    bottom = cd'*Hcd;
    ...
    if mod(i, opt_def.CG_restart) == 0
        r = matvec_Hx(kern, g_kern, model, data, HI, x) + grad;
    end
